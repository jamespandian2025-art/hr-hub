      ...
      <aside style={{ width: 232, minWidth: 232, flexShrink: 0, paddingRight: 28, position: 'sticky', top: 0 }}>
        ...
        <div style={{ display: 'flex', alignItems: 'center', gap: 10, marginBottom: 20 }}>
          ...
          <div>
            ...
            <div style={{ fontSize: 11, color: '#9ca3af' }}>Enterprise OS</div>
          </div>
        </div>
        ...
        <p style={{ fontSize: 12, color: '#6b7280', lineHeight: 1.6, marginBottom: 24 }}>A comprehensive design system for a modern enterprise operating system.</p>

        {/* Colors */}
        <div style={{ marginBottom: 24 }}>
          <div style={{ fontSize: 10, fontWeight: 700, color: '#9ca3af', letterSpacing: '1px', textTransform: 'uppercase', marginBottom: 12 }}>Colors</div>
            ...
            <div style={{ display: 'flex', gap: 3 }}>
              {palette.green.map(s => (
                <div key={s.label} style={{ flex: 1 }}>
                  ...
                  <div style={{ fontSize: 9, color: '#9ca3af', textAlign: 'center', marginTop: 3 }}>{s.label}</div>
                </div>
              ))}
            </div>
          </div>
              ...
              {palette.semantic.map(s => (
                <div key={s.label} style={{ display: 'flex', alignItems: 'center', gap: 7 }}>
                  ...
                  <div>
                    ...
                    <div style={{ fontSize: 10, color: '#9ca3af' }}>{s.hex.toUpperCase()}</div>
                  </div>
                </div>
              ))}
            </div>
            ...
            <div style={{ display: 'flex', gap: 2 }}>
              {palette.neutral.map(s => (
                <div key={s.label} style={{ flex: 1 }}>
                  ...
                  <div style={{ fontSize: 8, color: '#9ca3af', textAlign: 'center', marginTop: 2 }}>{s.label}</div>
                </div>
              ))}
            </div>
          </div>
        ...
        <div>
          ...
          <div style={{ fontSize: 16, fontWeight: 700, color: '#111827', marginBottom: 3 }}>Unbounded</div>
          ...
          <div style={{ display: 'grid', gap: 5 }}>
            {typeScale.map(t => (
              <div key={t.name} style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', padding: '4px 0', borderBottom: '1px solid #f3f4f6' }}>
                ...
                <span style={{ fontSize: 10, color: '#9ca3af' }}>{t.size}/{t.line}</span>
                <span style={{ fontSize: 10, color: '#9ca3af' }}>{t.weight}</span>
              </div>
            ))}
          </div>
        </div>
      ...
      <div style={{ flex: 1, minWidth: 0 }}>
          ...
          <div style={{ display: 'grid', gap: 12 }}>
            {[
              ...
              <div key={row.label} style={{ display: 'flex', alignItems: 'center', gap: 10 }}>
                <span style={{ width: 72, fontSize: 12, color: '#6b7280', flexShrink: 0 }}>{row.label}</span>
            ...
            <div style={{ display: 'flex', alignItems: 'center', gap: 10 }}>
              <span style={{ width: 72, fontSize: 12, color: '#6b7280', flexShrink: 0 }}>Icon Button</span>
        ...
        <Section title="Inputs">
          <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr 1fr', gap: 14 }}>
            <InputField label="Text Input" placeholder="Enter text" rightIcon={<ChevronDown size={14} color="#9ca3af" />} />
            <InputField label="Focused" placeholder="Enter text" focused />
            <InputField label="Dropdown" placeholder="Select an option" rightIcon={<ChevronDown size={14} color="#9ca3af" />} />
            <div>
              ...
              <div style={{ display: 'flex', alignItems: 'center', border: '1px solid #e5e7eb', borderRadius: 8, padding: '0 12px', height: 40, background: '#fff', gap: 8 }}>
                <span style={{ flex: 1, fontSize: 13, color: '#9ca3af' }}>Select options</span>
            ...
            <div>
              ...
              <div style={{ display: 'flex', alignItems: 'center', border: '1px solid #e5e7eb', borderRadius: 8, padding: '0 12px', height: 40, background: '#fff', gap: 8 }}>
                ...
                <span style={{ flex: 1, fontSize: 13, color: '#9ca3af' }}>Search anything...</span>
              </div>
            </div>
            <div>
        ...
        <Section title="Table">
          <div style={{ border: '1px solid #e5e7eb', borderRadius: 12, overflow: 'hidden' }}>
            <table style={{ width: '100%', borderCollapse: 'collapse' }}>
              <thead>
                <tr style={{ background: '#f9fafb', borderBottom: '1px solid #e5e7eb' }}>
                  ...
                  <th style={{ padding: '10px 14px', fontSize: 11, fontWeight: 600, color: '#6b7280', textAlign: 'left', textTransform: 'uppercase', letterSpacing: '0.05em' }}>Item Name</th>
                  <th style={{ padding: '10px 14px', fontSize: 11, fontWeight: 600, color: '#6b7280', textAlign: 'left', textTransform: 'uppercase', letterSpacing: '0.05em' }}>SKU</th>
                  <th style={{ padding: '10px 14px', fontSize: 11, fontWeight: 600, color: '#6b7280', textAlign: 'left', textTransform: 'uppercase', letterSpacing: '0.05em' }}>Category</th>
                  <th style={{ padding: '10px 14px', fontSize: 11, fontWeight: 600, color: '#6b7280', textAlign: 'left', textTransform: 'uppercase', letterSpacing: '0.05em' }}>Status</th>
                  <th style={{ padding: '10px 14px', fontSize: 11, fontWeight: 600, color: '#6b7280', textAlign: 'right', textTransform: 'uppercase', letterSpacing: '0.05em' }}>Price</th>
                </tr>
              </thead>
              <tbody>
                {tableItems.map((item, i) => (
                  <tr key={i} style={{ borderBottom: '1px solid #f3f4f6' }}>
                    ...
                    <td style={{ padding: '12px 14px', fontSize: 13, color: '#6b7280' }}>{item.sku}</td>
                    <td style={{ padding: '12px 14px', fontSize: 13, color: '#6b7280' }}>{item.cat}</td>
                    <td style={{ padding: '12px 14px' }}>
            ...
            <div style={{ padding: '10px 14px', display: 'flex', justifyContent: 'space-between', alignItems: 'center', borderTop: '1px solid #f3f4f6' }}>
              <span style={{ fontSize: 12, color: '#6b7280' }}>Showing 1 to 4 of 1,248 items</span>
              <div style={{ display: 'flex', alignItems: 'center', gap: 4 }}>
                <PageBtn><ChevronLeft size={13} /></PageBtn>
                <PageBtn active>1</PageBtn>
                <PageBtn>2</PageBtn>
                <PageBtn>3</PageBtn>
                <span style={{ fontSize: 12, color: '#9ca3af', padding: '0 4px' }}>â€¦</span>
                <PageBtn>312</PageBtn>
        ...
        <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 14, marginBottom: 14 }}>
          <Section title="Badges &amp; Status" noMargin>
            <div style={{ display: 'grid', gap: 14 }}>
              ...
              <div>
                <div style={{ fontSize: 12, color: '#6b7280', marginBottom: 8 }}>Count Badge</div>
            ...
            <div style={{ display: 'grid', gap: 10 }}>
              <div style={{ border: '1px solid #e5e7eb', borderRadius: 12, padding: '16px 18px' }}>
                <div style={{ display: 'flex', gap: 14, alignItems: 'center' }}>
                  ...
                  <div>
                    <div style={{ fontSize: 12, color: '#9ca3af', marginBottom: 2 }}>Total Items</div>
                    ...
                    <div style={{ fontSize: 11, color: '#9ca3af' }}>Across all categories</div>
                  </div>
                </div>
              </div>
              <div style={{ border: '1px solid #e5e7eb', borderRadius: 12, padding: '16px 18px' }}>
                <div style={{ display: 'flex', gap: 14, alignItems: 'center' }}>
                  ...
                  <div>
                    <div style={{ fontSize: 12, color: '#9ca3af', marginBottom: 2 }}>Total Cost</div>
                    <div style={{ fontSize: 22, fontWeight: 700, color: '#111827', letterSpacing: '-0.5px', lineHeight: 1 }}>PHP 2,000,000.00</div>
                    <div style={{ fontSize: 11, color: '#9ca3af', marginTop: 3 }}>Base price total</div>
                  </div>
                </div>
              </div>
            </div>
        ...
        <Section title="Icons">
          <div style={{ display: 'grid', gap: 14 }}>
            <div>
              <div style={{ fontSize: 11, color: '#9ca3af', fontWeight: 600, textTransform: 'uppercase', letterSpacing: '0.06em', marginBottom: 8 }}>Navigation</div>
            ...
            <div>
              <div style={{ fontSize: 11, color: '#9ca3af', fontWeight: 600, textTransform: 'uppercase', letterSpacing: '0.06em', marginBottom: 8 }}>Actions</div>
              <div style={{ display: 'flex', gap: 8, flexWrap: 'wrap' }}>
            ...
            <div>
              <div style={{ fontSize: 11, color: '#9ca3af', fontWeight: 600, textTransform: 'uppercase', letterSpacing: '0.06em', marginBottom: 8 }}>Status</div>
              <div style={{ display: 'flex', gap: 8, flexWrap: 'wrap' }}>
          ...
          <div style={{ display: 'grid', gap: 12 }}>
            ...
            <div style={{ display: 'flex', alignItems: 'center', gap: 8 }}>
              <div style={{ display: 'flex', alignItems: 'center', gap: 6, fontSize: 13, color: '#374151', border: '1px solid #e5e7eb', borderRadius: 8, padding: '6px 12px', cursor: 'pointer' }}>
                10 / page <ChevronDown size={13} color="#9ca3af" />
              </div>
            </div>
          </div>
        </Section>
        ...
        <Section title="Tabs">
          <div style={{ borderBottom: '1px solid #e5e7eb', display: 'flex', gap: 0 }}>
            {tabs.map(t => (
              <button key={t} onClick={() => setActiveTab(t)} style={{ padding: '8px 16px', fontSize: 13, fontWeight: activeTab === t ? 600 : 400, color: activeTab === t ? green : '#6b7280', border: 'none', borderBottom: `2px solid ${activeTab === t ? green : 'transparent'}`, background: 'transparent', cursor: 'pointer', transition: 'color 150ms' }}>
                {t}
              </button>
            ))}
          </div>
          ...
          <Section title="Drawer (Right Panel)" noMargin>
            <div style={{ border: '1px solid #e5e7eb', borderRadius: 14, overflow: 'hidden' }}>
              <div style={{ padding: '12px 16px', borderBottom: '1px solid #e5e7eb', display: 'flex', alignItems: 'center', justifyContent: 'space-between' }}>
                ...
                <X size={15} color="#9ca3af" style={{ cursor: 'pointer' }} />
              </div>
              <div style={{ borderBottom: '1px solid #e5e7eb', padding: '0 16px', display: 'flex', gap: 0 }}>
                {drawerTabs.map(t => (
                  <button key={t} onClick={() => setDrawerTab(t)} style={{ padding: '8px 12px', fontSize: 12, fontWeight: drawerTab === t ? 600 : 400, color: drawerTab === t ? green : '#6b7280', border: 'none', borderBottom: `2px solid ${drawerTab === t ? green : 'transparent'}`, background: 'transparent', cursor: 'pointer' }}>
                    {t}
                  </button>
                ))}
              </div>
...
function Section({ title, children, noMargin }: { title: string; children: React.ReactNode; noMargin?: boolean }) {
  return (
    <div style={{ background: '#fff', border: '1px solid #e5e7eb', borderRadius: 14, padding: '18px 20px', marginBottom: noMargin ? 0 : 14 }}>
      <div style={{ fontSize: 10, fontWeight: 700, color: '#9ca3af', letterSpacing: '1px', textTransform: 'uppercase', marginBottom: 14 }}>{title}</div>
      {children}
    </div>
  )
}
  ...
  return (
    <div style={{ display: 'flex', gap: 10, alignItems: 'flex-start', background: '#fff', border: '1px solid #e5e7eb', borderRadius: 9, padding: '10px 12px', boxShadow: '0 2px 8px rgba(0,0,0,0.06)' }}>
      ...
      <div style={{ flex: 1 }}>
        ...
        <div style={{ fontSize: 11, color: '#6b7280', marginTop: 1, lineHeight: 1.4 }}>{msg}</div>
      </div>
      <X size={12} color="#9ca3af" style={{ cursor: 'pointer', flexShrink: 0, marginTop: 1 }} />
    </div>
  )
}
