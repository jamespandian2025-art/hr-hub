  ...
  return (
    <main style={{ fontFamily: font, padding: '0 20px 32px', minHeight: '100vh' }}>
      <div style={{ padding: '20px 0 18px', display: 'flex', alignItems: 'flex-start', justifyContent: 'space-between', gap: 14, flexWrap: 'wrap' }}>
        <div>
          <div style={{ fontSize: 12, color: '#64748b', marginBottom: 12 }}>HR Hub &gt; Approvals &gt; Deleted Approvals</div>
          <h1 style={{ margin: 0, color: '#0f172a', fontSize: 24 }}>Deleted Approvals</h1>
          <p style={{ margin: '6px 0 0', color: '#64748b', fontSize: 13 }}>Restore approval requests or delete them forever.</p>
        </div>
              ...
              {filtered.map(row => {
                ...
                return (
                  <tr key={row.id} style={{ borderTop: '1px solid #f1f5f9' }}>
                    ...
                    <td style={tdStyle}><span style={avatarStyle}>{initials(row.employeeName)}</span><span><strong style={{ display: 'block' }}>{row.employeeName}</strong><small style={{ color: '#64748b' }}>{row.employeeCode}</small></span></td>
...
function EmptyState({ icon, title, text }: { icon: React.ReactNode; title: string; text: string }) {
  return <div style={{ padding: 44, textAlign: 'center', color: '#94a3b8' }}><div style={{ margin: '0 auto 12px', width: 54, height: 54, borderRadius: '50%', background: '#f1f5f9', display: 'grid', placeItems: 'center' }}>{icon}</div><strong style={{ display: 'block', color: '#0f172a', fontSize: 15 }}>{title}</strong><p style={{ margin: '6px 0 0', fontSize: 13 }}>{text}</p></div>
}
...
const thStyle = { padding: '12px 14px', textAlign: 'left' as const, color: '#64748b', fontSize: 11, fontWeight: 900, background: '#fbfdff' }
