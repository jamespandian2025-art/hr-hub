...
const hrRecordsUseCompanyContext = /(?:companyIdFromRequest|resolveCompanyId)\s*\(\s*request\s*\)/.test(hrRecordsRoute)
const hrRecordItemsUseCompanyContext = /(?:companyIdFromRequest|resolveCompanyId)\s*\(\s*request\s*\)/.test(hrRecordsItemRoute)
check('HR records API verifies tenant membership', /assertCompanyAccess/.test(hrRecordsRoute) && hrRecordsUseCompanyContext && /assertCompanyAccess/.test(hrRecordsItemRoute) && hrRecordItemsUseCompanyContext)
